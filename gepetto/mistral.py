...
class MistralModelSync():
    name = "Minxie"
    def get_token_price(self, token_count, direction="output", model_engine="meta-llama/Meta-Llama-3-70B-Instruct"):
        return (0.50 / 1000000) * token_count

    def chat(self, messages, temperature=0.7, model="meta-llama/Meta-Llama-3-70B-Instruct"):
        """Chat with the model.
    ...
    def function_call(self, messages = [], tools = [], temperature=0.7, model="meta-llama/Meta-Llama-3-70B-Instruct"):
        api_key = os.getenv("ANYSCALE_API_KEY")
        api_base = os.getenv("ANYSCALE_BASE_URL")
        client = OpenAI(api_key=api_key, base_url=api_base)
        response = client.chat.completions.create(
